...
make_ensemble_emp <- function(ensemble_name, K, M, Y, train_indices, component_pdfs, 
  ...
  if(ensemble_name=="TLP") {
    ...
    model {
      ...
      for (i in 1:n) {
        ...
        for (k in 1:K) {
          ...
          print(H_b[i, k]);
          print(\"Hs_truncated: \");
          print((H_a[i, k]*0.9945) + 0.0001);
          print((H_b[i, k]*0.9945) + 0.0001);
          log_weighted_mixt_cdf_a[k] += beta_lcdf((H_a[i, k] * 0.9945) + 0.0001| alpha[k], beta[k]);
          log_weighted_mixt_cdf_b[k] += beta_lcdf((H_b[i, k] * 0.9945) + 0.0001| alpha[k], beta[k]);
        }
    ...
    init_vals <- list(alpha=rnorm(K,1,1e-3),
                      beta=rnorm(K,1,1e-3),
    ...
    model {
      ...
      for (i in 1:n) {
        ...
        for (k in 1:K) {
          ...
          print(H_b[i]);
          print(\"Hs_truncated: \");
          print((H_a[i]*0.9945) + 0.0001);
          print((H_b[i]*0.9945) + 0.0001);
          log_weighted_mixt_cdf_a[k] += beta_lcdf((H_a[i] * 0.9945) + 0.0001| alpha[k], beta[k]);
          log_weighted_mixt_cdf_b[k] += beta_lcdf((H_b[i] * 0.9945) + 0.0001| alpha[k], beta[k]);
        }
        target += log_diff_exp(log_sum_exp(log_weighted_mixt_cdf_a), log_sum_exp(log_weighted_mixt_cdf_b));
      }
    }
    ...
    parameters {
      vector<lower=0.0001, upper=0.9999>[K] mu;
      vector<lower=0.0001>[K] nu;
      simplex[M] omega_k[K];
      simplex[K] w;
    }
