...
extract_dis <- function(filename){
  ...
  dis$season <- ifelse(as.numeric(substr(file_only, 3, 4))>18,
                       paste0(as.numeric(substr(file_only, 6, 9)),"/",as.numeric(substr(file_only, 6, 9))+1),
                       paste0(as.numeric(substr(file_only, 6, 9))-1,"/",as.numeric(substr(file_only, 6, 9))))
  dis$model_name <- substr(file_only, 11, gregexpr("\\.", file_only)[[1]]-1)
  return(dis)
}
...
write.csv(tmp.df, file="calibration_work/BLPdata_app/rawdata/allDis.csv", quote = FALSE, row.names = FALSE)


###-------for real time comparison---------#
## get all model files without the metadata
some_files2 <- c(list.files("model-forecasts/real-time-component-models/", full.names=TRUE, recursive = TRUE),
                 list.files("model-forecasts/real-time-ensemble-models/", full.names=TRUE, recursive = TRUE))
...
write.csv(tmp2.df, file="calibration_work/BLPdata_app/rawdata/rtDis.csv", quote = FALSE, row.names = FALSE)

#-----------leave this out for now--------------------------------#

# ## get all model files without the metadata
# some_files3 <- list.files("model-forecasts/component-models/", full.names=TRUE, recursive = TRUE)
# some_files3 <- some_files3[-grep("metadata.txt", some_files3)]
# some_files3 <- some_files3[-grep("models-for-2017-2018", some_files3)]
# some_files3 <- some_files3[-grep("model-id-map", some_files3)]
# some_files3 <- some_files3[-grep("complete-modelids", some_files3)]
# tmp3 <- lapply(some_files3, FUN=extract_point_ests)
# tmp3.df <- do.call(rbind.data.frame, tmp3)
# ## get all model files without the metadata
# some_files4 <-list.files("model-forecasts/real-time-component-models/", full.names=TRUE, recursive = TRUE)
# some_files4 <- some_files4[-grep("metadata.txt", some_files4)]
# 
# ## extract point estimates and put into one dataframe
# tmp4 <- lapply(some_files4, FUN=extract_point_ests)
# tmp4.df <- do.call(rbind.data.frame, tmp4)
# write.csv(rbind(tmp3.df,tmp4.df), file="BLPdata_app/rawdata/mean_forecasts.csv", quote = FALSE, row.names = FALSE)
