...
# plot_PDFs_bin <- function(Y,comp_dens,forecast_names, yaxis){
#   bin_num <- length(unique(comp_dens$bin_start))
...
#     as.matrix()
#   # get index
#   for (i in 1:ncol(comp_dens_m)){
#     barplot(comp_dens_m, 
#          main = paste0("test_component",i),xlab="",cex.main=0.8, ylim=yaxis,space=rep(0,bin_num))
#   }
#   # ensembles
#   for (ens in forecast_names){
#     barplot(ord_y,get(paste0(ens,"_results_",scenario))$test_dens[order(Y[!ind_est])], type="l",
#          main = paste0("test_",ens),xlab="",cex.main=0.8, ylim=yaxis)
#     
#   }
# }

plot_PITs_bin <- function(comp_probs,forecast_names,index,train=TRUE){
  name_list <- replace(forecast_names, forecast_names %in% c("EW_BMC1","BMC1"), 
                       c("BLP","EW_BLP"))
  test_ind <- c(setdiff(1:n2,index))
  ...
  if(train){
    ...
    for (i in 1:ncol(comp_probs)){
      hist(comp_probs[index,i], breaks = 0:10/10, col = grey(0.5), freq = FALSE, 
           ylim = c(0, 9), main = paste0("train_component ",i),xlab="",cex.main=0.8)
      abline( h = 1, lty = 2)
      }
      # ensembles
    for (j in 1:length(forecast_names)){
      hist(get(paste0(name_list[j],"_results_binned"))$pit_train, breaks = 0:10/10, 
           col = grey(0.5), freq = FALSE, ylim = c(0, 9), main = paste0("train_",name_list[j]),
           xlab="",cex.main=0.8)
      abline( h = 1, lty = 2)
      }
    } else{
      for (i in 1:ncol(comp_probs)){
        hist(comp_probs[test_ind,i],breaks = 0:10/10, col = grey(0.5), 
             freq = FALSE, 
             ylim = c(0, 9), main = paste0("test_component ",i),xlab="",cex.main=0.8)
        abline( h = 1, lty = 2)
        }
        # ensembles
      for (j in 1:length(forecast_names)){
        hist(get(paste0(name_list[j],"_results_binned"))$pit_test, breaks = 0:10/10, 
             col = grey(0.5), freq = FALSE, ylim = c(0, 9), main = paste0("test_",name_list[j]),
             xlab="",cex.main=0.8)
        abline( h = 1, lty = 2)
        }
  }
