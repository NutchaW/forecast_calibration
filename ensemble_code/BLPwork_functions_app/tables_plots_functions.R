####------------------------------------ Cramer distance function -------------------------------------------------#####
cd_calc <- function(values) {
  # integration
  c <- ecdf(values)
  int_res <-
    integrate(function(x)
      ((c(x) - x) ^ 2), 0, 1, subdivisions = 2000)
  return(list(2, int_res[[1]]))
}
reli_cd <- function(file1,
                    file2,
                    test_seas,
                    test = TRUE,
                    cd_type = 1) {
...
make_sim_table <-
  function(forecast_names,
    ...
    # arrange
    load(paste0(path, "/data_transformed/model_list.rda"))
    params_LP2 <-
      read.csv(
        "~/git/end-of-2019-2020/cdc-flusight-ensemble/weights/target-based-weights.csv"
      ) %>%
      dplyr::filter(season == test_seasons[ts],
                    target == tar_names[tar])
    params_LP2$component_model_id[params_LP2$component_model_id == "CU-BMA"] <-
      "CUBMA"
    params_LP2$component_model_id[params_LP2$component_model_id == "CU-EAKFC_SEIRS"] <-
      "CU_EAKFC_SEIRS"
    params_LP2$component_model_id[params_LP2$component_model_id == "CU-EAKFC_SIRS"] <-
      "CU_EAKFC_SIRS"
    params_LP2$component_model_id[params_LP2$component_model_id == "CU-EKF_SEIRS"] <-
      "CU_EKF_SEIRS"
    params_LP2$component_model_id[params_LP2$component_model_id == "CU-EKF_SIRS"] <-
      "CU_EKF_SIRS"
    params_LP2$component_model_id[params_LP2$component_model_id == "CU-RHF_SEIRS"] <-
      "CU_RHF_SEIRS"
    params_LP2$component_model_id[params_LP2$component_model_id == "CU-RHF_SIRS"] <-
      "CU_RHF_SIRS"
    params_LP2$component_model_id[params_LP2$component_model_id == "Delphi-BasisRegression"] <-
      "Delphi_BasisRegression"
    params_LP2$component_model_id[params_LP2$component_model_id == "Delphi-EmpiricalFuture"] <-
      "Delphi_EmpiricalFutures"
    params_LP2$component_model_id[params_LP2$component_model_id == "Delphi-EmpiricalTraj"] <-
      "Delphi_EmpiricalTrajectories"
    params_LP2$component_model_id[params_LP2$component_model_id == "Delphi-DeltaDensity1"] <-
      "Delphi_ExtendedDeltaDensity"
    params_LP2$component_model_id[params_LP2$component_model_id == "Delphi-DeltaDensity2"] <-
      "Delphi_MarkovianDeltaDensity"
    params_LP2$component_model_id[params_LP2$component_model_id == "Delphi-Uniform"] <-
      "Delphi_Uniform"
    params_LP2$component_model_id[params_LP2$component_model_id == "LANL-DBMplus"] <-
      "LANL_DBMplus"
    params_LP2$component_model_id[params_LP2$component_model_id == "Protea-Cheetah"] <-
      "Protea_Cheetah"
    params_LP2$component_model_id[params_LP2$component_model_id == "Protea-Kudu"] <-
      "Protea_Kudu"
    params_LP2$component_model_id[params_LP2$component_model_id == "Protea-Springbok"] <-
      "Protea_Springbok"
    params_LP2$component_model_id[params_LP2$component_model_id == "ReichLab-KCDE_PHB"] <-
      "ReichLab_kcde_backfill_post_hoc"
    params_LP2$component_model_id[params_LP2$component_model_id == "ReichLab-KCDE_NB"] <-
      "ReichLab_kcde_backfill_none"
    params_LP2$component_model_id[params_LP2$component_model_id == "ReichLab-KDE"] <-
      "ReichLab_kde"
    params_LP2$component_model_id[params_LP2$component_model_id == "ReichLab-SARIMA1"] <-
      "ReichLab_sarima_seasonal_difference_FALSE"
    params_LP2$component_model_id[params_LP2$component_model_id == "ReichLab-SARIMA2"] <-
      "ReichLab_sarima_seasonal_difference_TRUE"
    params_LP2$component_model_id[params_LP2$component_model_id == "FSNetwork-TW"] <-
      "target-based-weights"
    params_LP2$component_model_id[params_LP2$component_model_id == "FSNetwork-EW"] <-
      "equal-weights"
    params_LP2$component_model_id[params_LP2$component_model_id == "FluOutlook-Mech"] <-
      "FluOutlook_Mech"
    params_LP2$component_model_id[params_LP2$component_model_id == "FluOutlook-MechAug"] <-
      "FluOutlook_MechAug"
    params_LP2$component_model_id[params_LP2$component_model_id == "FluX-ARLR"] <-
      "FluX_ARLR"
    params_LP2$component_model_id[params_LP2$component_model_id == "FluX-FluX_LSTM"] <-
      "FluX_LSTM"
    params_LP2$component_model_id[params_LP2$component_model_id == "UA-EpiCos"] <-
      "UA_EpiCos"
    ...
    table_params <-
      data.frame(matrix(NA, nrow = 6, ncol = length(param_cols)))
    colnames(table_params) <- param_cols
    table_params$Method <- forecast_names
    table_params <-
      table_params[c(ncol(table_params), 1:(ncol(table_params) - 1))]
    # params
    ...
    # fill tables
    non_mix <-
      forecast_names[forecast_names %in% c("BLP", "LP", "EW", "EW_BLP")]
    for (ens in non_mix) {
      for (i in 1:27) {
        if (ens == "LP") {
          table_params[table_params$Method == ens, paste0("omega_k[", 1, i, "]")] <-
            params2[i]
        } else if (ens == "EW") {
          table_params[table_params$Method == ens, paste0("omega_k[", 1, i, "]")] <-
            params1
          ...
          table_params[table_params$Method == ens, paste0("omega_k[", 1, i, "]")] <-
            params3[i + 2]
          table_params[table_params$Method == ens, paste0("alpha[", 1, "]")] <-
            params3[1]
          table_params[table_params$Method == ens, paste0("beta[", 1, "]")] <-
            params3[2]
        } else if (ens == "EW_BLP") {
          table_params[table_params$Method == ens, paste0("omega_k[", 1, i, "]")] <-
            params1
    ...
    for (ens in mix) {
      ...
      if (grepl("EW_BMC", ens, fixed = TRUE)) {
        for (k in 1:K) {
          table_params[table_params$Method == ens, paste0("alpha[", k, "]")] <-
            params6[k]
          table_params[table_params$Method == ens, paste0("beta[", k, "]")] <-
            params6[K + k]
          table_params[table_params$Method == ens, paste0("w[", k, "]")] <-
            params6[(2 * K) + k]
          for (m in 1:27) {
            table_params[table_params$Method == ens, paste0("omega_k[", k, m, "]")] <-
              params1
          }
        }
      } else {
    ...
    table_params <-
      table_params[rowSums(is.na(table_params)) < ncol(table_params),
...
distance_heatmap <- function(cv_score, pname) {
  ...
  cv_score %>%
    ...
    ungroup() %>%
    dplyr::mutate(wthin = ifelse(grepl("2", `Model Name`), 1, 0)) %>%
    ...
    scale_fill_manual(values = RColorBrewer::brewer.pal(9, "PuBu")[1:4],
                      name = "rank") +
    geom_text(aes(
      label = sprintf("%0.2f", round(`Mean validation log score`, 2)),
      colour = factor(wthin)
    ),
    size = 3,
    show.legend = FALSE) +
...
loc_heatmap <- function(loc_frame) {
  ...
  loc <- loc_frame %>%
    ...
    ungroup()
  ggplot(loc, aes(
    factor(model_name, levels = mod_order),
    factor(location, levels = lorder)
  )) +
    ...
    scale_fill_manual(values = RColorBrewer::brewer.pal(9, "PuBu")[c(2:7)],
                      name = "rank") +
    geom_text(aes(label = sprintf("%0.2f", round(loc_mean, 2))), size =
                3) +
    theme_bw() +
    theme(
      plot.title = element_text(size = 8),
...
reli_plot <- function(file1, file2, test_seas, test = TRUE) {
  ...
  ggplot(frame, aes(rand_pit, color = Method)) +
    ...
    facet_wrap( ~ target, ncol = 2) +
    labs(
      title = paste0("20", as.numeric(test_seas) - 1, "/20", test_seas),
      x = "PIT values",
      y = "CDF"
    ) +
    scale_colour_manual(
  ...
  function(file1,
    ...
    ggplot(frame, aes(rand_pit, color = Method)) +
      ...
      ggtitle(
        ifelse(
          test,
          "(b) Probability plots of ensemble forecasts in the test period",
...
mean_heat_all <- function(yscore) {
  ...
  yscore %>%
    ...
    scale_fill_manual(values = RColorBrewer::brewer.pal(9, "PuBu")[c(2:7)],
                      name = "rank") +
    geom_text(aes(label = sprintf("%0.2f", round(mean_across, 2))), size =
                3) +
    guides(fill = guide_legend(ncol = 6)) +
    theme(
...
mean_heat_all2 <- function(yscore, titlen) {
  ...
  yscore %>%
    ...
    theme(
      ...
      panel.background = element_blank(),
      plot.background = element_blank(),
      strip.text.x = element_text(margin = margin(.13, 0, .13, 0, "cm"), size =
                                    7)
    ) +
    scale_x_discrete(labels = name, expand = c(0, 0)) +
    scale_y_discrete(expand = c(0, 0))
