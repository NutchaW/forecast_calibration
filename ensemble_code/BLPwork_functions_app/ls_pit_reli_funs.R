...
reli_plot <- function(test_seas){
  some_files <- list.files(paste0("ensembles/pit_ls_frame/train_test/",test_seas,"/"),
                           full.names=TRUE, recursive = TRUE)
  tmp <- lapply(some_files, FUN=read.csv)
  frame1 <- do.call(rbind.data.frame, tmp)
  some_files1 <- list.files(paste0("ensembles/pit_ls_frame/train_test/EW_TLP/",test_seas,"/"),
                            full.names=TRUE, recursive = TRUE)
